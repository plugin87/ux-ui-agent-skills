...
`examples/terminal/` is Meridian Terminal, a trading desk: a market ticker,
candlesticks with volume and a 7-period moving average, an order book with depth
bars, a cumulative depth curve, time and sales, watchlist sparklines, a stacked
area of allocation drift, a P and L waterfall with connectors, a risk-return
bubble chart, a correlation matrix and a radial gauge. Every chart is inline SVG
drawn from `--color-chart-*`; there is no chart library anywhere in the repo.
Gate 40, and it passes all sixteen checks that apply to it in both themes,
including 280px at a 1.25x root font.

The geometry is generated by a seeded walk rather than hand-placed, so it can be
regenerated and cannot drift.

**`/data-dashboard`** is the new skill that produces this class of screen: the
layout sequence that avoids the equal-card wall, the geometry recipe per chart
type, the accessibility contract (what a chart's `desc` must actually say), and
the traps below, with their measured numbers.

**What the gates and a careful look each caught**

Gates:
- A chart colour behind 12px white initials: **2.65:1** in dark. A chart palette
  is tuned for large filled shapes, not text.
- Success green as chip text on a raised surface: **4.43:1**, four hundredths
  short. The colour moved to the icon and the sign; the text went neutral.
- Three dense tables overflowed 280px at a 1.25x root font. Each drops the one
  column a phone can lose - the sparkline, the derived value, the size - in a
  media query, with the reason in a comment.

A look, after all sixteen were green:
- The **stacked area had dark wedges between bands**: the reversed baseline was
  indexed by position instead of by coordinate.
- The **correlation matrix was not symmetric**, and its diagonal was not 1.
  Sixteen green gates and an instant tell to anyone who reads one for a living.
- The **waterfall had no connectors**, so it read as floating bars.

None of those three is measurable, which is the entire argument for rendering
